...
const EventTicket: FC<EventTicketProps> = ({
  event,
  isModalOpen,
  modalClose,
}) => {

  // Function to generate and download the ticket as PDF
  const downloadTicket = async () => {
    try {
      ...
      pdf.text(`Ticket ID: ${event._id.slice(-12).toUpperCase()}`, 105, 38, {
        align: "center",
      });
      ...
      pdf.setFont("helvetica", "bold");
      const priceText =
        event.productPrice > 0 ? `$${event.productPrice}` : "Free";
      ...
      pdf.setFontSize(8);
      pdf.setTextColor(107, 114, 128);
      pdf.text("Scan this code at the entrance", qrCenterX, qrY + 60, {
        align: "center",
      });
      ...
      pdf.text("For questions, contact support@coursedeep.com", 105, 280, {
        align: "center",
      });
      ...
      const filename = `${event.productTitle.replace(
        /[^a-zA-Z0-9]/g,
        "_"
      )}_Ticket.pdf`;
      pdf.save(filename);

      showSuccessToast("Ticket downloaded successfully");
      modalClose(false);

    } catch (error) {
      console.error("Error generating PDF:", error);
      showErrorToast("Failed to download ticket. Please try again.");
    }
  };
            ...
            <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
              ...
              <div className="bg-gradient-to-r from-yellow-300 via-yellow-500 to-yellow-300 py-4 sm:p-8 text-white relative overflow-hidden ">
                <div className="flex flex-col items-center relative z-10 space-y-1">
                  <h3 className="text-sm sm:text-lg font-semibold tracking-wider text-zinc-900">
                    EVENT TICKET
                  </h3>
                  ...
                  <p className="text-zinc-500 text-sm">
                    Ticket ID: {event._id.slice(-12).toUpperCase()}
                  </p>
